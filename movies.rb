...
LABELS = %i(link title year country date genre duration rating director main_actors).freeze
...
movies.uniq(&:director)
      ...
      .each { |movie| puts movie.director.to_s }

# Output of non-USA shot films amount
puts "\nAmount of non-USA shot films\n\n"
puts movies.reject { |movie| movie.country.include?('USA') }.size.to_s
...
movies.each { |e| puts e.inspect.to_s }
